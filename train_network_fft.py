...
class SHLDataset(Dataset):
    def __init__(self, dataset_type='train', positions=['Hips']):
        ...
        if dataset_type is 'train' or dataset_type is 'validation':
            ...
            for position in positions:
                feature_path = os.path.join(path, position, '{}_fft_features.csv'.format(dataset_type))
                label_path = os.path.join(path, position, '{}_labels.csv'.format(dataset_type))

                features = np.loadtxt(feature_path, delimiter=',', dtype=np.float32)[1:, ]
            ...
            feature_path = os.path.join(path, 'test_fft_features.csv')
            label_path = os.path.join(path, 'test_labels.csv')

            self.x_data = np.loadtxt(feature_path, delimiter=',', dtype=np.float32)[1:, ]
